...
As for the previous part, list item contains a lock that is initialized when the element is added. In this version, the thread is searching the place to add or delete without locking anything. When it arrives at the right place, it checks that the *previous->next == current* (i.e. that no other threads had inserted or removed an element just at the same time). It also checks that neither *previous* nor *current* are already locked. If the condition is *true*, it *atomically* checks the condition again and then if lock *previous* and *current* for the *insert* case.  After that it adds the new node and releases *current* and *previous*. If the condition is false, it just call the *insert* method once again. Concerning the *delete* case, after testing that it everything is unchanged and unlocked, it atomically removes the node. 
...
1. **Two threads try to write at the same place at the same time**
    
    The two thread can find the place at the same time, they can even execute the condition at the same time but only one can